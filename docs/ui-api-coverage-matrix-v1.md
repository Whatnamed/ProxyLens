...
| **Review: Background/Security Catalog Matches** | Provenance-backed process/path catalog matches | `GET /api/v1/intelligence/findings` | **READY — Phase 4C1** | 三条 Microsoft Defender embedded entries；process + event-time process_path；knowledge provenance；无 trust/signature/rule action |
