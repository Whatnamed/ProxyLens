...
2. **快捷窗口的本地天对齐 (Local-Day Aware)**:
   - **Today**: 当前本地日期的 `00:00:00` 至当前时间（转换为对应 UTC 请求 API）；
   - **Yesterday**: 昨天本地日期的 `00:00:00` 至 `23:59:59.999`；
   - **Last 7 Days**: 当前时间往前推 7 个整天（或近 7 个本地日零点）；
   - **Last 30 Days**: 当前时间往前推 30 个整天；
3. **半开区间原则**:
   - 所有的查询与聚合在语义上均遵循 `[from, to)` 半开区间。
