...
- 1.5M E-drive scale acceptance 与 Phase 3S 保持独立；本阶段未启动 Phase 4B2B/4C2、真实 Controller 或任何网络生命周期。
...
- Phase 4B2B 的 broader historical target 扩展与 Phase 4C2 rule suggestions 仍未开始；Phase 4C1 catalog 能力不替代这些后续契约。
...
- 本阶段未修改 accounting writer、migration、index、真实 Controller 或网络生命周期；Phase 4B2B/4C2 继续 Deferred。
...
### Phase 4E-C Production UI Continuation & Core V1 Closure (Complete)

- 在 `5288bc1` 基线完成 exact production preflight：current-user Task enabled/running、Interactive/Limited、exact Logon + Time trigger、schema 9、只读 `quick_check=ok`、active incremental-v2、磁盘 guard 与 config/credential metadata 均通过；
- 使用 normal current-user NSIS 完成正式 `unregister → graceful stop → replace → reconcile` upgrade；installed Supervisor/Runtime/Query API 与构建输出逐字节一致，desktop 仅保留 Tauri NSIS bundle marker 的 `UNK → NSS` 三字节打包归一化差异；config、autostart preference 与 credential presence 基线保持，未读取 Secret；
...
- 当前 installed Task 使用 GUI-subsystem `proxylens-supervisor-host.exe`，Runtime child 使用 creation-time no-console flags；后台 console closure 现在由进程创建边界与真实 current-user upgrade evidence 支撑，而不是 post-creation hide。公共签名、tag、GitHub Release 与外部 installer 发布仍属于后续 distribution 工作。
