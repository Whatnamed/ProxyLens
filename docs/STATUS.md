...
4. **write-quiescent low-disk mode**：Pre-start DiskGuard tripped 时不启动 collector、不启动 scheduler、shutdown 跳过 flush 循环与 truncate，日志明确 degraded/low-disk，保持 READY 提供只读 Query 服务；`seedPreflight` 与 `advanceIncrementalChunk` 统一使用 `DiskGuardStopFloorBytes`（1GiB）和 15% 比率下限；新增 mock/temp runtime targeted test。
5. **session progress 单调性保证**：`SQLiteEventSink.Emit` 更新 session 进度使用 `MAX(COALESCE(last_frame_sequence, 0), ?)`；`StateEngine` 增加 `EmitSessionHealth` 方法；`IngestJournalRecord` 针对带外 direct session evidence 自动继承当前游标；新增先高 frame sequence 后零 frame health 不下降回归。
...
7. **frame-complete publish boundary**：`extendCutToFrameEnd` 引入 `isFrameComplete`，当且仅当 live running session 的最新帧包含完成证据（如 `SamplingResidual`）或已有后续帧/session 已结束才允许切入，未完成帧一律安全回退到该帧之前；StateEngine 确保所有 snapshot frame 结尾具备完成证据；新增未完成帧拒绝切入、完成证据写入后整体发布并发回归测试。
