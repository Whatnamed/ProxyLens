...
// four Phase 4A detector families in memory. No finding-specific follow-up
// query is issued.
func (s *AuditIntelligenceService) ListFindings(ctx context.Context, filter AuditFindingFilter) (*AuditFindingResult, error) {
	...
	rows, err := s.db.QueryContext(ctx, fmt.Sprintf(`
		SELECT
			...
			process, host, sniff_host, destination_ip, network, rule, rule_payload
		FROM %s
		WHERE %s = ?
	...
	for kind, aggregates := range groups {
		...
		sort.Slice(filtered, func(i, j int) bool {
			...
			return auditSubjectKey(kind, filtered[i].subject) < auditSubjectKey(kind, filtered[j].subject)
		})
...
func scanAuditTrafficRow(rows sqlRows) (auditTrafficRow, error) {
	...
	var process, host, sniffHost, destinationIP, network, rule, rulePayload sql.NullString
	if err := rows.Scan(
		...
		&process, &host, &sniffHost, &destinationIP, &network, &rule, &rulePayload,
	); err != nil {
...
func buildAuditFinding(kind AuditFindingKind, a *auditAggregate) AuditFinding {
	...
	return AuditFinding{
		ID: findingID(kind, subject), Kind: kind, Subject: subject, Evidence: evidence,
