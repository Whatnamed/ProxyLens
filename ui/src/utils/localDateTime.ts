...
export function clampCalendarDay(year: number, month: number, day: number): Date {
  return new Date(year, month, Math.min(Math.max(day, 1), daysInCalendarMonth(year, month)));
}
